...
def load_test_dataset(n_node, k_sparse, device, start_node = None):
    if not os.path.isfile(f'../data/cvrp/testDataset-{n_node}.pt'):
        # Create directory if it doesn't exist
        os.makedirs('../data/cvrp', exist_ok=True)
        dataset = []
        for i in range(10):
        ...
        torch.save(dataset, f'../data/cvrp/testDataset-{n_node}.pt')
    else:
        dataset = torch.load(f'../data/cvrp/testDataset-{n_node}.pt', map_location=device)
...
def load_val_dataset(n_node, k_sparse, device, start_node = None):
    if not os.path.isfile(f'../data/cvrp/valDataset-{n_node}.pt'):
        # Create directory if it doesn't exist
        os.makedirs('../data/cvrp', exist_ok=True)
        dataset = []
        for i in range(100):
            ...
            dataset.append(instance)
        dataset = torch.stack(dataset)
        torch.save(dataset, f'../data/cvrp/valDataset-{n_node}.pt')
    else:
        dataset = torch.load(f'../data/cvrp/valDataset-{n_node}.pt', map_location=device)
...
if __name__ == '__main__':
    ...
    for n in [100]: # problem scale, 500, 1000, 2000
