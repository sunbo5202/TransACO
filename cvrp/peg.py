...
def log_aco_to_peg(paths, costs, iteration, instance_id=0, alltime_best_cost=None):
    """
    Log ACO paths to PEG graph for CVRP (minimization problem).
    
    Args:
        paths: torch tensor with shape (max_seq_len, n_ants) for CVRP
        costs: torch tensor with shape (n_ants,)
        iteration: current iteration number
        instance_id: instance identifier for multiple problem instances
        alltime_best_cost: Historical best (lowest) cost value (for tracking convergence)
    """
    global ACO_PEG
    ...
    # Ensure paths is 2D and costs is 1D
    ...
    if len(costs.shape) != 1:
        if costs.dim() == 0:
            print(f"ERROR: costs is scalar, expected 1D tensor (iteration={iteration}, instance_id={instance_id})")
            return None
        costs = costs.squeeze()
        if len(costs.shape) != 1:
            print(f"ERROR: costs.shape={costs.shape} after squeeze, expected 1D tensor (iteration={iteration}, instance_id={instance_id})")
            return None
    
    # For CVRP, paths shape should be (max_seq_len, n_ants)
    # If it's (n_ants, max_seq_len), transpose it
    original_shape = paths.shape
    if paths.shape[0] < paths.shape[1]:
        # Paths is likely (n_ants, max_seq_len), need to transpose to (max_seq_len, n_ants)
        paths = paths.T
        print(f"Debug: Transposed paths from {original_shape} to {paths.shape} (iteration={iteration}, instance_id={instance_id})")
    
    # Final validation
    if paths.shape[1] != costs.shape[0]:
        print(f"ERROR: Mismatch between paths.shape[1]={paths.shape[1]} and costs.shape[0]={costs.shape[0]} (iteration={iteration}, instance_id={instance_id})")
        return None
    
    # Find best path in current iteration
    ...
    for ant_id in range(paths.shape[1]):
        ...
        # Calculate pheromone contribution value (1.0/cost, same as in ACO)
        contribution = 1.0 / costs[ant_id].item() if costs[ant_id].item() > 0 else 0.0
        ...
        ACO_PEG.add_node(
            ...
            ant_id=ant_id,
            cost=costs[ant_id].item(),
            is_best=is_best,
            path=path_list,
            alltime_best_cost=alltime_best_cost if alltime_best_cost is not None else costs[ant_id].item(),  # Store historical best (lowest)
        )
...
def visualize_aco_peg(save_path=None, instance_id=None, layout_type='hierarchical'):
    """
    Visualize ACO PEG graph with improved layout options inspired by EvoMapX.
    ...
    # Prepare node colors - use cost-based coloring
    node_colors = []
    node_sizes = []
    costs = [subgraph.nodes[n].get('cost', 0) for n in subgraph.nodes()]
    ...
    if costs:
        ...
        for node in subgraph.nodes():
            cost = subgraph.nodes[node].get('cost', 0)
            # Normalize cost to [0, 1] for colormap
            normalized_cost = (cost - min_cost) / cost_range if cost_range > 0 else 0
            node_colors.append(normalized_cost)
    ...
    nx.draw_networkx_nodes(
        ...
        cmap='viridis_r',  # Lower cost = darker color
        alpha=0.8,
        edgecolors='black',
        linewidths=1.5,
        ax=ax
    ...
    if costs:
        ...
        cbar.set_label('Path Cost', rotation=270, labelpad=20, fontsize=12, fontweight='bold')
    
    # Set title and labels
    title = f"ACO Population Evolution Graph (PEG) - CVRP"
    if instance_id is not None:
...
def visualize_aco_peg_simple(save_path=None, instance_id=None):
    ...
    title = "ACO Population Evolution Graph (PEG) - CVRP"
    if instance_id is not None:
        title += f" - Instance {instance_id}"
    title += " (Spring Layout)"
...
def analyze_aco_peg_metrics(instance_id=None):
    ...
    best_costs_per_iter = []  # Use historical best (lowest) cost for CVRP
    avg_costs_per_iter = []
    alltime_best_per_iter = []  # Track historical best per iteration
    
    for iter_num in iterations:
        iter_nodes = [n for n in subgraph.nodes() 
                     if subgraph.nodes[n].get('iteration') == iter_num]
        if iter_nodes:
            iter_costs = [subgraph.nodes[n].get('cost', 0) for n in iter_nodes]
            # Use historical best if available, otherwise use current generation best
            iter_alltime_best = [subgraph.nodes[n].get('alltime_best_cost', None) for n in iter_nodes]
            if any(best is not None for best in iter_alltime_best):
                # Get all non-None historical best values and use the minimum (lowest cost)
                valid_bests = [b for b in iter_alltime_best if b is not None]
                iter_alltime_best_val = min(valid_bests)
                alltime_best_per_iter.append(iter_alltime_best_val)
            else:
                # Fallback to current generation best (lowest)
                iter_alltime_best_val = min(iter_costs)
                alltime_best_per_iter.append(iter_alltime_best_val)
            
            # Use historical best (lowest) for convergence analysis (monotonically decreasing for minimization)
            best_costs_per_iter.append(iter_alltime_best_val)
            avg_costs_per_iter.append(np.mean(iter_costs))
    
    # Ensure historical best is monotonically decreasing (for minimization problem)
    # This handles any recording inconsistencies
    if len(best_costs_per_iter) > 1:
        # Make historical best monotonically non-increasing
        for i in range(1, len(best_costs_per_iter)):
            if best_costs_per_iter[i] > best_costs_per_iter[i-1]:
                # Current value is higher, use previous value (shouldn't happen for correct recording)
                best_costs_per_iter[i] = best_costs_per_iter[i-1]
    
    if len(best_costs_per_iter) > 1:
        metrics['initial_best_cost'] = best_costs_per_iter[0]
        metrics['final_best_cost'] = best_costs_per_iter[-1]
        metrics['total_improvement'] = best_costs_per_iter[0] - best_costs_per_iter[-1]  # initial - final for CVRP (minimization)
        metrics['relative_improvement'] = (metrics['total_improvement'] / best_costs_per_iter[0]) * 100 if best_costs_per_iter[0] > 0 else 0
        
        # Convergence rate (how fast it improves)
        improvements = [best_costs_per_iter[i] - best_costs_per_iter[i+1] 
                       for i in range(len(best_costs_per_iter)-1)]
    ...
    if len(best_costs_per_iter) > 1:
        cost_variance_per_iter = []
        for iter_num in iterations:
            iter_nodes = [n for n in subgraph.nodes() 
                         if subgraph.nodes[n].get('iteration') == iter_num]
            if iter_nodes:
                iter_costs = [subgraph.nodes[n].get('cost', 0) for n in iter_nodes]
                cost_variance_per_iter.append(np.var(iter_costs))
        
        metrics['avg_cost_variance'] = np.mean(cost_variance_per_iter) if cost_variance_per_iter else 0
        metrics['diversity_trend'] = 'increasing' if cost_variance_per_iter[-1] > cost_variance_per_iter[0] else 'decreasing'
    ...
    for iter_num in iterations:
        ...
        if iter_best_nodes:
            current_best_cost = subgraph.nodes[iter_best_nodes[0]].get('cost', 0)
            if prev_best_cost is not None and current_best_cost != prev_best_cost:
                best_path_changes += 1
...
def explain_aco_peg_behavior(instance_id=None):
    ...
    explanation = "=" * 80 + "\n"
    explanation += "ACO SEARCH BEHAVIOR ANALYSIS (Based on Population Evolution Graph)\n"
    explanation += "=" * 80 + "\n\n"
    ...
    if 'total_improvement' in metrics:
        ...
        explanation += f"   - Initial best cost: {metrics['initial_best_cost']:.2f}\n"
        explanation += f"   - Final best cost: {metrics['final_best_cost']:.2f}\n"
    ...
    # Diversity analysis
    if 'avg_cost_variance' in metrics:
        explanation += "5. POPULATION DIVERSITY:\n"
        explanation += f"   - Average cost variance: {metrics['avg_cost_variance']:.2f}\n"
...
def run_aco_with_peg(aco, n_iterations, inference=False, instance_id=0, start_iteration=0):
    """
    Wrapper function to run CVRP ACO and log to PEG graph.
    This function replicates aco.run() but adds PEG logging.
    
    Args:
        aco: ACO instance (for CVRP)
    ...
    Returns:
        Same as aco.run() - lowest_cost
    """
    for i in range(n_iterations):
        paths = aco.gen_path(require_prob=False)
        costs = aco.gen_path_costs(paths)
        
        if aco.adaptive:
            aco.improvement_phase(paths, costs)
        
        if aco.swapstar:
            indexes = costs.topk(8, largest=False).indices
            aco.multiple_swap_star(paths, indexes=indexes)
            costs = aco.gen_path_costs(paths)
        
        improved = False
        best_cost, best_idx = costs.min(dim=0)
        if best_cost < aco.lowest_cost:
            aco.shortest_path = paths[:, best_idx].clone()
            aco.lowest_cost = best_cost
            if aco.adaptive:
                aco.intensification_phase(paths, costs, best_idx)
            if aco.min_max:
                max_val = aco.problem_size / aco.lowest_cost
                if aco.max is None:
                    aco.pheromone *= max_val / aco.pheromone.max()
                aco.max = max_val
            improved = True
        
        if not aco.adaptive or improved:
            aco.update_pheronome(paths, costs)
            if aco.adaptive:
                aco.elite_pool.insert(0, (aco.shortest_path, aco.lowest_cost))
                if len(aco.elite_pool) > 5:  # pool_size = 5
                    del aco.elite_pool[5:]
        else:
            aco.diversification_phase()
        
        # Log to PEG AFTER local search and pheromone update
        # paths shape is (max_seq_len, n_ants) for CVRP
        # Pass historical best (lowest) cost for convergence tracking
        # Handle lowest_cost which can be float('inf') initially or a tensor
        if isinstance(aco.lowest_cost, torch.Tensor):
            lowest_cost_val = aco.lowest_cost.item()
        else:
            lowest_cost_val = aco.lowest_cost
        
        # Ensure we use the actual lowest cost (handle inf case and ensure it's truly the minimum)
        current_best_cost = costs.min().item()
        if lowest_cost_val == float('inf'):
            # Not initialized yet, use current best
            lowest_cost_val = current_best_cost
        else:
            # Use the minimum of historical best and current best (for safety)
            lowest_cost_val = min(lowest_cost_val, current_best_cost)
        ...
        log_aco_to_peg(
            ...
            instance_id=instance_id,
            alltime_best_cost=lowest_cost_val  # Pass historical best (lowest) cost
        )
    
    return aco.lowest_cost
