...
class ACO():
    ...
    def __init__(self,
        ...
        if heuristic is None:
            EPS = 1e-10
            # Avoid division by zero: add small epsilon to distances
            self.heuristic = self.prizes.unsqueeze(0) / (self.distances + EPS)
        else:
    ...
    def sparsify(self, k_sparse):
        ...
        # Avoid division by zero: add small epsilon to distances
        self.heuristic = self.prizes.unsqueeze(0) / (sparse_distances + EPS)
    ...
    def pick_node(self, mask, cur_node, require_prob):
        ...
        # Handle NaN and Inf values first (compatible with older PyTorch versions)
        dist = torch.where(torch.isnan(dist), torch.tensor(EPS, device=dist.device), dist)
        dist = torch.where(torch.isinf(dist), torch.tensor(EPS, device=dist.device), dist)
        
        # Ensure non-negative values
        dist = torch.clamp(dist, min=0.0)
        
        # Ensure each row has at least one positive value
        # If all values are zero (or very small), ensure dummy node has a small probability
        row_sums = dist.sum(dim=1, keepdim=True)
        all_zero_rows = (row_sums < EPS).squeeze()
        if all_zero_rows.any():
            # For rows with all zeros, set dummy node to a small positive value
            dist[all_zero_rows, -1] = EPS
        
        # Add small epsilon only to unmasked positions to avoid exact zeros (helps with numerical stability)
        # This ensures masked positions stay at 0, while unmasked positions get a small boost
        dist = dist + EPS * mask
