...
def infer_instance(model, instance, k_sparse, n_ants, t_aco_diff, enable_peg=False, instance_id=0, save_peg_dir=None):
    """
    ...
    if model:
        ...
        pm_mat, hm_mat = model(pyg_data, distances)
        aco = ACO(distances, prizes, max_len[len(prizes)], n_ants, 
                 pheromone=pm_mat, heuristic=hm_mat, device=device)
    else:
...
@torch.no_grad()
def test(dataset, model, n_ants, k_sparse, t_aco, enable_peg=False, save_peg_dir=None, peg_instances=None):
    """
...
def main(n_node, model_file=None, k_sparse=None, n_ants=20, t_aco=None, enable_peg=False, 
         peg_dir=None, num_test_instances=None, peg_instances=None):
    """
    Main test function for OP.
    
    Args:
        n_node: Problem scale
        model_file: Path to model checkpoint (None for baseline)
        k_sparse: Sparse parameter
        n_ants: Number of ants
        t_aco: List of iteration counts to test
    ...
    k_sparse = k_sparse or sparse_table.get(n_node, 20)
    t_aco = t_aco or [10]
...
if __name__ == "__main__":
    ...
    parser.add_argument("-k", "--k_sparse", type=int, default=None,
                       help="K-sparse parameter (default: based on problem size)")
    ...
    if model_file is None:
        ...
        possible_paths = [
            f'./pretrained/op/op{opt.nodes}.pt',  # Same directory
            f'../pretrained/op/op{opt.nodes}.pt',  # Parent directory
            f'../../pretrained/op/op{opt.nodes}.pt',  # Two levels up
        ]
        model_file = None
        for path in possible_paths:
            if os.path.isfile(path):
                ...
                break
        if model_file is None:
            print(f"Model file 'op{opt.nodes}.pt' not found in any of the following paths:")
            for path in possible_paths:
                print(f"  - {path}")
            print("Will only run baseline.")
    
    main(opt.nodes, model_file, opt.k_sparse, opt.n_ants, 
