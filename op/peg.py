...
def log_aco_to_peg(paths, objs, iteration, instance_id=0, alltime_best_obj=None):
    """
    Log ACO paths to PEG graph for OP (maximization problem).
    
    Args:
        paths: torch tensor with shape (max_len, n_ants) for OP
        ...
        alltime_best_obj: Historical best objective value (for tracking convergence)
    """
    global ACO_PEG
    ...
    # For OP, paths shape is (max_len, n_ants) after gen_sol, then transposed to (n_ants, max_len) in run()
    # We need to transpose back to (max_len, n_ants) for consistent format
    original_shape = paths.shape
    if paths.shape[0] > paths.shape[1]:
        # Paths is likely (max_len, n_ants), which is correct
        pass
    else:
        # Paths is likely (n_ants, max_len), need to transpose
        paths = paths.T
    ...
    # Find best path in current iteration (OP maximizes rewards, so use argmax)
    ...
    for ant_id in range(paths.shape[1]):
        ...
        # For OP, reward value (higher is better) - store as 'cost' for compatibility (but it's actually reward)
        # In visualization, we'll use 'viridis' (not 'viridis_r') since higher is better
        ...
        ACO_PEG.add_node(
            ...
            is_best=is_best,
            path=path_list,
            alltime_best_obj=alltime_best_obj if alltime_best_obj is not None else objs[ant_id].item(),  # Store historical best
        )
...
def visualize_aco_peg(save_path=None, instance_id=None, layout_type='hierarchical'):
    """
    Visualize ACO PEG graph for OP (maximization problem).
    ...
    # Prepare node colors - use reward-based coloring (higher reward = better color for OP)
    node_colors = []
    node_sizes = []
    ...
    nx.draw_networkx_nodes(
        ...
        cmap='viridis',  # Higher reward = brighter color (not reversed for OP)
        alpha=0.8,
        edgecolors='black',
        linewidths=1.5,
        ax=ax
    ...
    title = f"ACO Population Evolution Graph (PEG) - OP"
    if instance_id is not None:
...
def visualize_aco_peg_simple(save_path=None, instance_id=None):
    ...
    title = "ACO Population Evolution Graph (PEG) - OP"
    if instance_id is not None:
        title += f" - Instance {instance_id}"
    title += " (Spring Layout)"
...
def analyze_aco_peg_metrics(instance_id=None):
    ...
    For OP, this analyzes reward maximization.
    ...
    # 3. Reward evolution analysis (for OP, we maximize rewards)
    iterations = sorted(set(subgraph.nodes[n].get('iteration', 0) 
                           for n in subgraph.nodes()))
    metrics['total_iterations'] = len(iterations)
    
    best_rewards_per_iter = []  # For OP, higher is better - use historical best
    ...
    for iter_num in iterations:
        ...
        if iter_nodes:
            ...
            iter_alltime_best = [subgraph.nodes[n].get('alltime_best_obj', None) for n in iter_nodes]
            if any(best is not None for best in iter_alltime_best):
                # Get all non-None historical best values and use the maximum
                valid_bests = [b for b in iter_alltime_best if b is not None]
                iter_alltime_best_val = max(valid_bests)
                alltime_best_per_iter.append(iter_alltime_best_val)
            else:
                # Fallback to current generation best
                iter_alltime_best_val = max(iter_rewards)
                alltime_best_per_iter.append(iter_alltime_best_val)
            
            # Use historical best for convergence analysis (monotonically increasing for maximization)
            best_rewards_per_iter.append(iter_alltime_best_val)
            avg_rewards_per_iter.append(np.mean(iter_rewards))
    
    # Ensure historical best is monotonically increasing (for maximization problem)
    # This handles any recording inconsistencies
    if len(best_rewards_per_iter) > 1:
        # Make historical best monotonically non-decreasing
        for i in range(1, len(best_rewards_per_iter)):
            if best_rewards_per_iter[i] < best_rewards_per_iter[i-1]:
                # Current value is lower, use previous value (shouldn't happen for correct recording)
                best_rewards_per_iter[i] = best_rewards_per_iter[i-1]
    
    if len(best_rewards_per_iter) > 1:
        metrics['initial_best_reward'] = best_rewards_per_iter[0]
        metrics['final_best_reward'] = best_rewards_per_iter[-1]
        metrics['total_improvement'] = best_rewards_per_iter[-1] - best_rewards_per_iter[0]  # final - initial for OP
...
def explain_aco_peg_behavior(instance_id=None):
    ...
    explanation = "=" * 80 + "\n"
    explanation += "ACO SEARCH BEHAVIOR ANALYSIS (Based on Population Evolution Graph) - OP\n"
    explanation += "=" * 80 + "\n\n"
...
def run_aco_with_peg(aco, n_iterations, inference=False, instance_id=0, start_iteration=0):
    """
    Wrapper function to run OP ACO and log to PEG graph.
    This function replicates aco.run() but adds PEG logging.
    
    Args:
        aco: ACO instance (for OP)
        n_iterations: Number of iterations to run
        inference: Whether in inference mode (not used for OP)
    ...
    for i in range(n_iterations):
        sols = aco.gen_sol(require_prob=False)  # Returns (max_len, n_ants)
        objs = aco.gen_sol_obj(sols)  # Returns (n_ants,)
        sols = sols.T  # Transpose to (n_ants, max_len) for consistency with run()
        
        best_obj, best_idx = objs.max(dim=0)  # OP maximizes rewards
        if best_obj > aco.alltime_best_obj:
            aco.alltime_best_obj = best_obj
            aco.alltime_best_sol = sols[best_idx]
            if aco.min_max:
                max_val = aco.alltime_best_obj * aco.n * aco.Q
                if aco.max is None:
                    aco.pheromone *= max_val/aco.pheromone.max()
                aco.max = max_val
        aco.update_pheronome(sols, objs, best_obj, best_idx)
        
        # Log to PEG AFTER pheromone update
        # sols is (n_ants, max_len), transpose back to (max_len, n_ants) for PEG
        log_aco_to_peg(
            sols.T,  # Back to (max_len, n_ants)
            objs,    # (n_ants,) - reward values
            iteration=start_iteration + i, 
            instance_id=instance_id,
            alltime_best_obj=aco.alltime_best_obj.item() if isinstance(aco.alltime_best_obj, torch.Tensor) else aco.alltime_best_obj  # Pass historical best
        )
