...
def load_test_dataset(n_node, k_sparse, device):
    ...
    test_file = f'./data/op/testDataset-{n_node}.pt'
    if not os.path.isfile(test_file):
        # Create directory if it doesn't exist
        os.makedirs('./data/op', exist_ok=True)
        # Generate test dataset with 10 instances
        coor_list = []
        for _ in range(10):
            coor = torch.rand(size=(n_node, 2))
            coor_list.append(coor)
        val_tensor = torch.stack(coor_list)
        torch.save(val_tensor, test_file)
        print(f"Generated test dataset with 10 instances: {test_file}")
    else:
        val_tensor = torch.load(test_file, map_location=device)
