...
def train_instance(model, optimizer, prize, weight, n_ants):
    ...
    phe_mat, heu_mat = model(prize, weight)#.reshape((prize.size(0), prize.size(0)))
    #heu_mat = model(src).reshape((prize.size(0), prize.size(0)))
    # heu_mat = torch.tensor(heu_mat, device=device)
...
def infer_instance(model, prize, weight, n_ants):
    model.eval()
    src = gen_pyg_data(prize, weight)
    phe_mat, heu_mat = model(prize, weight)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--nodes", metavar='N', type=int, default=300, help="Problem scale")
    ...
    parser.add_argument("-e", "--epochs", type=int, default=12, help="Epochs to run")
    parser.add_argument("-t", "--test_size", type=int, default=10, help="Number of instances for testing")
    parser.add_argument("-o", "--output", type=str, default="../pretrained/mkp",
