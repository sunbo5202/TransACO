...
class Net(nn.Module):
    ...
    def __init__(self, n_layers=6, n_embed=128, node_dim=6, n_head=8, normalization="batch", masked=None):
        super(Net, self).__init__()
        self.proj = nn.Linear(node_dim, n_embed)
        self.depot_proj = nn.Linear(6, n_embed)
    ...
    def forward(self, prize, weight):
        # weight: N 5
        prize = prize.to(torch.float32)
        weight = weight.to(torch.float32)
        node_feature = torch.cat((prize.unsqueeze(1), weight), dim=1) #[N,6]
        node_feature = self.proj(node_feature)
        x = node_feature
        ...
        hm = F.softmax(att_heu, dim=-1) + prize / weight.sum(dim=1)

        # _, indices = torch.topk(dist, 490, -1)
