...
class ACO():
    ...
    def __init__(self,  # constraints are set to n//2 after norm
        ...
        if pheromone is None:
            # Initialize as (n, n), will be expanded to (n+1, n+1) in add_dummy_node()
            self.pheromone = torch.ones(size=(self.n, self.n), device=device)
            if min_max:
                self.pheromone = self.pheromone * self.min
        else:
            # If pheromone is provided, it should be (n, n) shape
            # It will be expanded to (n+1, n+1) in add_dummy_node()
    ...
    def add_dummy_node(self):
        ...
        # Check if pheromone already has dummy node (shape is (n+1, n+1))
        # If shape is (n, n), we need to add dummy node
        if self.pheromone.shape == (self.n, self.n):
            # Pheromone is (n, n), need to add dummy node
            pheromone_added_row = torch.cat((self.pheromone, torch.zeros((1, self.n), device=self.device)), dim=0)
            self.pheromone = torch.cat((pheromone_added_row, 1e-10*torch.ones((self.n+1, 1), device=self.device)), dim=1)
        elif self.pheromone.shape == (self.n+1, self.n+1):
            # Pheromone already has dummy node, no need to modify
            pass
        else:
            # Unexpected shape, raise error
            raise RuntimeError(f"Unexpected pheromone shape: {self.pheromone.shape}, expected ({self.n}, {self.n}) or ({self.n+1}, {self.n+1})")
