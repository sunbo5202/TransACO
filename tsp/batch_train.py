...
def train_instance(model, optimizer, data, n_ants):
    ...
    optimizer.zero_grad()
    sum_loss.backward()
    torch.nn.utils.clip_grad_norm_(parameters=model.parameters(), max_norm=4.0, norm_type=2)
    optimizer.step()
...
def generate_traindata(count, n_node, k_sparse):
    ...
    x = torch.rand(size=(count, n_node, 2), device=device)
    n_nodes = n_node
    distances = torch.norm(x.unsqueeze(1) - x.unsqueeze(2), dim=-1, p=2)
    distances[:, torch.arange(n_nodes), torch.arange(n_nodes)] = 1e9  # note here
    return Data(x=x), distances
...
def train(n_node, n_ants, steps_per_epoch, epochs, k_sparse=None, batch_size=3, test_size=None, pretrained=None,
