...
class EncoderBlock(nn.Module):
    ...
    def forward(self, x, dist=None, cosine_sim=None):
        ...
        if self.normalization == "layer":
            ...
            x= x + self.att(x, x, x, dist) ++ self.att(x,x,x,cosine_sim)
            x = x + self.mlp(x)
        return x
...
class Net(nn.Module):
    ...
    def forward(self, pyg, dist=None):
        # x: B N 2
        x = pyg.x #[100,2]
        N = x.size(-2)
        x1 = x
        x2 = x1.unsqueeze(2)  # Shape: (B, N, 1, E)
        x3 = x1.unsqueeze(1)  # Shape: (B, 1, N, E)
        cosine_sim = F.cosine_similarity(x2, x3, dim=-1)  # Shape: (B, N, N)
        if dist is None:
            dist = torch.norm(x[:, None] - x, dim=2, p=2)
            dist[torch.arange(N), torch.arange(N)] = 1e9
        x = self.proj(x) #[100,128]
        # x: B N E
        for block in self.encoder:
            x = block(x, dist, cosine_sim) #[100,128]
