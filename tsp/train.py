...
def train_instance(model, optimizer, data, n_ants):
    ...
    for pyg_data, distances in data:
        ...
        sum_loss = sum_loss + reinforce_loss
        count = count + 1
...
def generate_traindata(count, n_node, k_sparse):
    for _ in range(count):
        ...
        yield gen_pyg_data(instance, k_sparse=k_sparse, start_node=None)
...
def train(n_node, n_ants, steps_per_epoch, epochs, k_sparse = None, batch_size = 3, test_size = None, pretrained = None, savepath = "../pretrained/tsp_nls"):
    k_sparse = k_sparse or n_node // 20
    net = Net().to(device)
    if pretrained:
        net.load_state_dict(torch.load(pretrained, map_ocation=device))
    optimizer = torch.optim.AdamW(net.parameters(), lr=lr)
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, epochs)
    val_list = load_val_dataset(n_node, k_sparse, device, start_node=None)
    if test_size is not None:
        val_list = val_list[:test_size]
    with open('Trans_tsp1000.txt', 'w') as f:
        stats = validation(n_ants, -1, net, val_list)
        val_results = [stats]
        best_result = (stats[-1], stats[-2], stats[-3])
        print(f'epoch 0:', stats)
        f.write(f'{stats[1]}\n')
        torch.save(net.state_dict(), os.path.join(savepath, f'tsp2opt{n_node}-best.pt'))
        ...
        for epoch in range(1, epochs+1):
            ...
            torch.save(net.state_dict(), os.path.join(savepath, f'tsp2opt{n_node}-last.pt'))

    print('\ntotal training duration:', sum_time)
    
    return f'../pretrained/tsp/tsp{n_node}.pt'

if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser()
    parser.add_argument("--nodes", metavar='N', type=int, default=20, help="Problem scale")
    ...
    parser.add_argument("-a", "--ants", type=int, default=40, help="Number of ants (in ACO algorithm)")
